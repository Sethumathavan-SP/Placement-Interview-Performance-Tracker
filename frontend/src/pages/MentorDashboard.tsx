    ...
    <div className="space-y-6">
              ...
              {studentsNeedingPlans.map((m) => (
                <div key={m.studentId} className="px-5 py-3 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg text-xs font-medium transition-colors bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50"
                  >
      ...
      <div className="flex flex-wrap items-center gap-3">
        <input
          ...
          className="text-sm border border-slate-300 rounded-lg px-3 py-1.5 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
        />
        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="text-sm border border-slate-300 rounded-lg px-3 py-1.5 focus:outline-none focus:border-blue-500">
        ...
        <select value={filterPriority} onChange={(e) => setFilterPriority(e.target.value)} className="text-sm border border-slate-300 rounded-lg px-3 py-1.5 focus:outline-none focus:border-blue-500">
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center"><p className="text-slate-500">No interventions match filters.</p></div>
      ) : (
                ...
                <button onClick={() => onToggleExpand(intv.interventionId)} className="w-full px-6 py-4 text-left hover:bg-slate-50/50 transition-colors">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-2 ml-4 flex-shrink-0">
                      <button
                        onClick={(e) => { e.stopPropagation(); onCreateIntervention(intv.studentId); }}
                        disabled={generating === intv.studentId}
                        className="px-2.5 py-1 rounded-lg text-xs font-medium transition-colors bg-amber-50 text-amber-700 border border-amber-200 hover:bg-amber-100 disabled:opacity-50"
                      >
                        {generating === intv.studentId ? 'Re-analyzing...' : 'Re-analyze'}
                      </button>
